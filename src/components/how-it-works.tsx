          ...
          {steps.map((item, index) => (
            <div key={item.step} className="relative text-center">
              ...
              {index < steps.length - 1 && (
                <div className="absolute top-6 left-[calc(50%+2rem)] hidden h-px w-[calc(100%-4rem)] bg-befit-green-light/30 lg:block" />
              )}
              <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-befit-green text-sm font-bold text-white">
                {item.step}
              </div>
              <h3 className="mt-4 text-sm font-semibold text-befit-dark">
                {item.title}
              </h3>
